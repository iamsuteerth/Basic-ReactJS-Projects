...
function MovieDetails({ selectedId, onCloseMovie, onAddWatched, watched }) {
  ...
  // Since we want the title update on mount
  useEffect(
    function () {
      if (!title) return;
...
export default function App() {
  ...
  // This is a custom useState hook useLocalStorageState
  const [watched, setWatched] = useLocalStorageState([], "watched");
  // How to use effects?
  // Register an effect (the side effect) to run on mount
  ...
  function handleAddWatch(movie) {
    ...
    // Access to this in all browsers
  ...
  // This CAN be replaced with an EventHandler function
  const { movies, isLoading, error } = useMovies(query, handleCloseMovie);
  // Effect rendered AFTER render

  /* Passing elements as props instead of implicit children props
    <Box
      element={
        <> 
          <WatchedSummary watched={watched} />
